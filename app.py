...
# UI mínima (sin márgenes, sin header/footer)
st.markdown("<style>.block-container{padding:0} header,footer{display:none}</style>", unsafe_allow_html=True)

# Inyectamos HTML+JS (ZXing corre 100% en el navegador; no requiere paquetes nativos)
st.html(f"""
<!doctype html>
<html lang="es">
<head>
...
<body>
  ...
  <script>
    ...
    async function warmUpWithFacingMode() {{
      // Pre-permiso + preview para desbloquear enumerateDevices completo
      const constraints = {{
        video: {{
    ...
    async function tryDecodeWithConstraints() {{
      // Camino ideal: constraints + facingMode environment
    ...
    async function startScan() {{
      try {{
        ...
        // 2) En Chrome móvil: intenta deviceId primero (más estable)
        if (isChromeMobile()) {{
          try {{
        ...
        alert("No se pudo iniciar la cámara. Verifica permisos del sitio para la cámara y vuelve a intentar.");
      }}
    }}
